...
def get_signal(df):
    ...
    buy_rsi  = cr > 45 and cr < RSI_OB
    sell_rsi = cr < 55 and cr > RSI_OS

    buy_macd  = cml > cms and cmh > 0
    sell_macd = cml < cms and cmh < 0
    ...
    log.info(
        f"Conditions | buy_cross={buy_cross} sell_cross={sell_cross} | "
        f"bull={bull} bear={bear} | "
        f"buy_rsi={buy_rsi} sell_rsi={sell_rsi} | "
        f"buy_macd={buy_macd} sell_macd={sell_macd}"
    )
...
def format_message(direction, info):
    ...
    return (
        f"{emoji} <b>GOLD {direction} SIGNAL {arrow}</b>\n"
        f"━━━━━━━━━━━━━━━━━\n"
